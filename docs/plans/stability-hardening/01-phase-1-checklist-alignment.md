...
`docs/plans/stability-hardening/00-plan.md` の Phase 1 に従い、実装状態と進捗チェックリストのずれをなくす。

対象は主に次の 3 系統。

- `docs/plans/c-binding-remediation/checklists/handles-status.csv`
- `docs/plans/c-binding-remediation/checklists/static-handle-table-status.csv`
...
2. Batch A の前に、`docs/guides/` に C ABI ownership/lifetime guide の雛形を作るか判断する。
