# 111 Phase 2 Batch DF: canvas annotation data rules

開始時刻: 2026-05-11 10:05:48 JST

## 目的

`110-phase-2-batch-de-runtime-effect-builder-generated-handles.md` の「次バッチ候補」から、`SkCanvas` drawAnnotation の data handle 0 / invalid handle と data pointer optional 規約を追加確認する。data handle 0 は optional data として許可し、registry に存在しない非 0 handle は no-op にする。

## 変更内容

- `skia/capi/sk_canvas.cpp`
  - `has_optional_data_handle` を追加し、`SkCanvas_drawAnnotation` で data handle 0 は許可、非 0 は registry 実体必須にした。
- `skia/capi/sk_canvas.h`
  - `SkCanvas_drawAnnotation` の required rect/key、optional data handle、invalid input no-op 規約を明記した。
- `skia/test/test_canvas_invalid_input_smoke.cpp`
  - handle 0、未知の非 0 handle、valid data handle、data pointer optional の smoke ケースを追加した。
- `docs/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_canvas.*` の P2 記録を Batch DF に更新した。
...
- `SkCanvas` clipShader shader handle 0 / invalid handle 規約を追加確認する。
