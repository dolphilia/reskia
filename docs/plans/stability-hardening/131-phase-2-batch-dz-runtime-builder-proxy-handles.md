# 131 Phase 2 Batch DZ: runtime builder proxy handles

開始時刻: 2026-05-12 23:06:53 JST

## 目的

`128-phase-2-batch-dw-runtime-builder-spans.md` の「次バッチ候補」から、`SkRuntimeEffect` builder uniform / child returned builder proxy handle ownership と delete 規約を再確認する。proxy handle は builder 側の storage に借用依存するが、handle 自体は呼び出し側が delete できることを固定する。
...
- `skia/capi/sk_runtime_effect_builder.h`
  - `uniform` / `child` の returned proxy handle ownership と invalid input 0 規約を明記した。
- `skia/test/test_runtime_effect_invalid_input_smoke.cpp`
  - uniform を持つ shader builder に対して `SkRuntimeShaderBuilder_uniform(builder, "u")` が registry 上の有効 proxy handle を返し delete できることを確認した。
  - child を持つ shader builder に対して `SkRuntimeShaderBuilder_child(builder, "child")` が registry 上の有効 proxy handle を返し delete できることを確認した。
  - 既存の zero/invalid string_view handle smoke と合わせて、invalid input 0 規約を固定した。
- `docs/plans/c-binding-remediation/checklists/capi-status.csv`
  - runtime builder 系の P2 記録を Batch DZ に更新した。
