# 113 Phase 2 Batch DH: runtime effect uniform and child spans

開始時刻: 2026-05-11 10:05:48 JST

## 目的

`112-phase-2-batch-dg-image-scale-pixels.md` の「次バッチ候補」から、`SkRuntimeEffect` uniforms / children returned handle ownership smoke を追加確認する。戻り値は effect に借用依存する span handle だが、handle 自体は呼び出し側が delete できることを固定する。

## 変更内容

- `skia/capi/sk_runtime_effect.h`
  - `SkRuntimeEffect_uniforms` / `SkRuntimeEffect_children` の borrowed span handle と delete 規約を明記した。
- `skia/test/test_runtime_effect_invalid_input_smoke.cpp`
  - uniform を持つ shader effect と child shader を持つ shader effect を作り、`uniforms` / `children` の戻り handle が registry に存在し delete できることを確認した。
- `docs/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_runtime_effect.h` の P2 記録を Batch DH に更新した。
