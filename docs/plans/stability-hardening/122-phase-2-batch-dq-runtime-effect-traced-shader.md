# 122 Phase 2 Batch DQ: runtime effect traced shader handles

開始時刻: 2026-05-11 10:30:44 JST

## 目的

`121-phase-2-batch-dp-image-async-callback-context.md` の「次バッチ候補」から、`SkRuntimeEffect` makeTracedShader の coordinate/data/child pointer required と trace handle 0 規約を再確認する。現状 C API の露出は `SkRuntimeEffect_MakeTraced` で、shader handle と trace coordinate pointer が必須のため、invalid input を handle 0、valid input を caller-owned trace handle として固定する。

## 変更内容

- `skia/capi/sk_runtime_effect.h`
  - `SkRuntimeEffect_MakeTraced` の returned trace handle ownership と invalid input handle 0 規約を明記した。
- `skia/test/test_runtime_effect_invalid_input_smoke.cpp`
  - zero / invalid shader handle と NULL trace coordinate に加え、valid shader + valid trace coordinate が registry 上の trace handle を返し、caller が delete できることを確認した。
- `docs/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_runtime_effect.*` の P2 記録を Batch DQ に更新した。
...
- `SkCanvas` drawDrawable / drawPicture の optional matrix/paint と NULL receiver 規約を再確認する。
- `SkImage` readPixels pixmap / raw buffer の nullable context と rowBytes 規約を再確認する。
- `SkRuntimeEffect` builder uniform / child lookup name handle 0 / invalid handle 規約を再確認する。
