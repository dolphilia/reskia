# 128 Phase 2 Batch DW: runtime builder span handles

開始時刻: 2026-05-11 10:59:49 JST

## 目的

`127-phase-2-batch-dv-image-peek-scale-pixmap.md` の「次バッチ候補」から、`SkRuntimeEffect` builder uniforms / children returned handle ownership と delete 規約を再確認する。returned handle は builder / effect に借用依存する中身を指すが、handle 自体は呼び出し側が delete できることを固定する。
...
- `skia/capi/sk_runtime_effect_builder.h`
  - uniforms / children の returned handle ownership と delete 規約を明記した。
- `skia/test/test_runtime_effect_invalid_input_smoke.cpp`
  - shader / color filter / blend builder の uniforms / children returned handle が delete できることを確認した。
  - uniform を持つ shader builder の uniforms handle、child shader を持つ shader builder の children handle が registry に存在し delete できることを確認した。
- `docs/plans/c-binding-remediation/checklists/capi-status.csv`
  - runtime builder 系の P2 記録を Batch DW に更新した。
...
- `SkImage` makeSubset / withDefaultMipmaps / makeRasterImage returned handle ownership と failure 0 規約を再確認する。
- `SkRuntimeEffect` builder uniform / child returned builder proxy handle ownership と delete 規約を再確認する。
