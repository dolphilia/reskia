# 93 Phase 2 Batch CN: image shader input rules

開始時刻: 2026-05-11 05:38:11 JST

## 目的

`92-phase-2-batch-cm-canvas-paint-path-region.md` の「次バッチ候補」から、`SkImage` shader / rawShader 系の required sampling と image pointer guard を追加確認する。既存実装の NULL guard を smoke test で固定し、公開ヘッダのコメントを invalid input returns 0 規約に揃える。

## 変更内容

- `skia/capi/sk_image.h`
  - shader / rawShader 系 API の image、sampling、local matrix の required/optional 規約と invalid input returns 0 を明記した。
- `skia/test/test_image_surface_invalid_input_smoke.cpp`
  - `SkImage_makeShader*` / `SkImage_makeRawShader*` の NULL image、NULL sampling、required matrix NULL smoke ケースを追加した。
- `docs/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_image.h` の P2 記録を Batch CN に更新した。
