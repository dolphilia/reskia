# 121 Phase 2 Batch DP: image async callback context rules

開始時刻: 2026-05-11 10:30:44 JST

## 目的

`120-phase-2-batch-do-canvas-matrix-transforms.md` の「次バッチ候補」から、`SkImage` asyncRescaleAndReadPixels callback context の fail callback / NULL context 規約を再確認する。invalid input では callback がある場合だけ NULL result callback を呼び、context は NULL を許可してそのまま callback に渡す。

## 変更内容

- `skia/capi/sk_image.h`
  - asyncRescaleAndReadPixels 系の context nullable と fail callback 規約を明記した。
- `skia/test/test_image_surface_invalid_input_smoke.cpp`
  - invalid input + NULL callback が no-op であること、invalid input + NULL context が NULL result callback として呼ばれることを smoke で固定した。
- `docs/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_image.*` の P2 記録を Batch DP に更新した。
