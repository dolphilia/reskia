# 123 Phase 2 Batch DR: canvas drawable and picture optional pointers

開始時刻: 2026-05-11 10:36:07 JST

## 目的

`122-phase-2-batch-dq-runtime-effect-traced-shader.md` の「次バッチ候補」から、`SkCanvas` drawDrawable / drawPicture の optional matrix / paint と NULL receiver 規約を再確認する。既存実装の NULL receiver / invalid handle no-op と optional matrix / paint を smoke test で固定する。

## 変更内容

- `skia/test/test_canvas_invalid_input_smoke.cpp`
  - `drawPicture` / `drawPicturePtrWithMatrixPaint` で valid matrix + NULL/valid paint の smoke ケースを追加した。
  - `drawDrawable` で valid matrix pointer の smoke ケースを追加した。
- `docs/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_canvas.*` の P2 記録を Batch DR に更新した。
