...
次候補は今回の `SkImage` getter/ref、`SkCanvas` pixel/draw、`SkCapabilities` と重複しないよう、詳細台帳の remaining representative / omitted 行から選ぶ。着手時は必ず `rg '<対象API名|detail id>' docs/plans/c-binding-remediation/checklists/capi-safety-detail-status.csv` で既存行を確認する。
