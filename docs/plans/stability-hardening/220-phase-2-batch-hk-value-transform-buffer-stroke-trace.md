...
`219-phase-2-batch-hj-value-icc-pdf-perlin.md` の「次バッチ候補」から、`SkPoint3` / `SkIPoint`、`SkRSXform`、`SkString`、`SkStrokeRec`、`SkTraceMemoryDump` の 5 件を順に確認する。NULL pointer、required output array、borrowed string lifetime、invalid enum、callback/dump no-op の規約を `docs/guides/c-api-safety-guidelines.md` に合わせて固定する。
