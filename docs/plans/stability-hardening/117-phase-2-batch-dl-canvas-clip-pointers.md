# 117 Phase 2 Batch DL: canvas clip pointer rules

開始時刻: 2026-05-11 10:20:52 JST

## 目的

`116-phase-2-batch-dk-runtime-effect-find-name-handles.md` の「次バッチ候補」から、`SkCanvas` clipRect / clipIRect / clipRRect / clipPath の pointer required と invalid input no-op 規約を再確認する。既存実装の NULL guard を smoke test で固定し、公開ヘッダのコメントを no-op 規約に揃える。

## 変更内容

- `skia/capi/sk_canvas.h`
  - clipRect / clipIRect / clipRRect / clipPath 系の required pointer と invalid input no-op 規約を明記した。
- `skia/test/test_canvas_invalid_input_smoke.cpp`
  - 各 clip pointer API の NULL 入力 no-op と valid pointer 入力の smoke ケースを追加した。
- `docs/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_canvas.*` の P2 記録を Batch DL に更新した。
