# 133 Phase 2 Batch EB: image shader ownership

開始時刻: 2026-05-12 23:28:38 JST

## 目的

`131-phase-2-batch-dz-runtime-builder-proxy-handles.md` の「次バッチ候補」から、`SkImage` shader / rawShader returned shader handle ownership と invalid input 0 規約を再確認する。localMatrix optional overload は NULL を許容し、成功時の shader handle は呼び出し側が delete できることを固定する。

## 変更内容

- `skia/capi/sk_image.h`
  - `SkImage_makeShader*` / `SkImage_makeRawShader*` の returned shader handle ownership と factory failure 0 規約を明記した。
  - optional localMatrix overload と required localMatrix overload の差をコメントで明確にした。
- `skia/test/test_image_surface_invalid_input_smoke.cpp`
  - `SkImage_makeShader(image, ..., nullptr)` が有効 shader handle を返し delete できることを確認した。
  - `SkImage_makeShaderWithSampling(image, sampling, nullptr)` が有効 shader handle を返し delete できることを確認した。
  - `SkImage_makeRawShader(image, ..., nullptr)` と `SkImage_makeRawShaderWithSampling(image, sampling, nullptr)` が有効 shader handle を返し delete できることを確認した。
- `docs/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_image.cpp` / `sk_image.h` の P2 記録を Batch EB に更新した。
