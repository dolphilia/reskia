# 99 Phase 2 Batch CT: image factory handle-zero rules

開始時刻: 2026-05-11 05:58:29 JST

## 目的

`98-phase-2-batch-cs-canvas-rrect-draw.md` の「次バッチ候補」から、`SkImage` makeSubset / withDefaultMipmaps / makeRasterImage 系の handle 0 return 規約を再確認する。Skia が NULL `sk_sp<SkImage>` を返した場合に registry 上の NULL image handle を作らず、C API の戻り値を `0` に正規化する。

## 変更内容

- `skia/capi/sk_image.cpp`
  - NULL `sk_sp<SkImage>` を `0` にする `make_image_handle` helper を追加した。
  - `SkImage_makeSubset` / `SkImage_makeSubsetWithRecorder` / `SkImage_withDefaultMipmaps` / `SkImage_makeNonTextureImage` / `SkImage_makeRasterImage` / `SkImage_makeRasterImageWithoutContext` を helper 経由にした。
- `skia/capi/sk_image.h`
  - 対象 factory API の invalid input / factory failure returns 0 規約を明記した。
- `skia/test/test_image_surface_invalid_input_smoke.cpp`
  - NULL image、NULL subset、factory failure subset、raster image valid handle smoke ケースを追加した。
- `docs/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_image.*` の P2 記録を Batch CT に更新した。
