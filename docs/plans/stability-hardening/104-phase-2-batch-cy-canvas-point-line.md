# 104 Phase 2 Batch CY: canvas point and line draw rules

開始時刻: 2026-05-11 06:04:05 JST

## 目的

`103-phase-2-batch-cx-runtime-effect-generated-handles.md` の「次バッチ候補」から、`SkCanvas` drawPoint / drawLine の point handle validity と paint required 規約を追加確認する。handle API では handle 0 だけでなく registry に存在しない非 0 handle も no-op にする。

## 変更内容

- `skia/capi/sk_canvas.cpp`
  - `has_point_handle` を追加し、`SkCanvas_drawPoint` / `SkCanvas_drawLine` で point handle の実体存在を確認するようにした。
- `skia/capi/sk_canvas.h`
  - `SkCanvas_drawPoint` / `SkCanvas_drawLine` / XY variants の required input と invalid input no-op 規約を明記した。
- `skia/test/test_canvas_invalid_input_smoke.cpp`
  - point handle 0、未知の非 0 handle、NULL paint、正常系の smoke ケースを追加した。
- `docs/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_canvas.*` の P2 記録を Batch CY に更新した。
