# 105 Phase 2 Batch CZ: canvas circle and arc draw rules

開始時刻: 2026-05-11 06:16:38 JST

## 目的

`104-phase-2-batch-cy-canvas-point-line.md` の「次バッチ候補」から、`SkCanvas` drawCircle / drawArc の point / rect / paint required 規約を追加確認する。`drawCircle` の handle API は handle 0 だけでなく registry に存在しない非 0 handle も no-op にする。

## 変更内容

- `skia/capi/sk_canvas.cpp`
  - `SkCanvas_drawCircle` を `has_point_handle` に揃え、未知の非 0 point handle を no-op にした。
- `skia/capi/sk_canvas.h`
  - `SkCanvas_drawArc` / `SkCanvas_drawCircle` / `SkCanvas_drawCircleAt` の required input と invalid input no-op 規約を明記した。
- `skia/test/test_canvas_invalid_input_smoke.cpp`
  - drawCircle / drawCircleAt / drawArc の invalid input no-op と正常系 smoke ケースを追加した。
- `docs/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_canvas.*` の P2 記録を Batch CZ に更新した。
