...
`docs/plans/c-binding-remediation/checklists/capi-safety-detail-status.csv` は、API 群と安全性規約単位で更新する。これは `capi-status.csv` より細かい重複防止用の台帳であり、同じファイル内の別 API 群を何度も再確認しないための一次参照にする。
