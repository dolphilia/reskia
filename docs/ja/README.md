...
運用手順の概要は `docs/ja/guides/skia-upgrade-workflow.md`、詳細な cycle 計画は `docs/ja/plans/skia-incremental-upgrade/00-continuous-upgrade-cycle.md` にあります。

## ライセンス

Reskia は BSD 3-Clause License で提供します。Skia 由来の source も同じ BSD-style license に従います。詳細は `LICENSE` と `skia/LICENSE` を参照してください。

third-party dependencies はそれぞれのライセンスに従います。`THIRD_PARTY_NOTICES.md` と `third_party/src/*` 配下の license file を参照してください。

## 主要ドキュメント

- `docs/ja/guides/repository-structure.md`: リポジトリ構造
- `docs/ja/guides/cmake-migration-guide.md`: CMake と依存解決モード
- `docs/ja/guides/architecture-overview.md`: アーキテクチャ概要
- `docs/ja/guides/c-api-bindings-overview.md`: C API / handle binding の全体像
- `docs/ja/guides/c-api-safety-guidelines.md`: C API 安全性規約
- `docs/ja/guides/public-api-coverage-workflow.md`: public API coverage の運用
- `docs/ja/guides/skia-upgrade-workflow.md`: Skia 追従 workflow
