...
調査方法: 既存 `README.md`、`docs/ja/guides`、`docs/ja/notes`、`docs/ja/plans`、`skia/CMakeLists.txt`、`cmake/reskia/*.cmake`、`cmake/deps/ReskiaDeps.cmake`、`skia/capi`、`skia/handles`、`third_party`、`vendor` を確認。

更新: 2026-06-02 に `bindings/` ディレクトリと root Rust workspace は削除された。Reskia 本体は Skia の CMake 再構成と C ABI 提供に集中し、言語別 wrapper はリポジトリ外で扱う前提に変更された。

## 1. このプロジェクトが目指していること

Reskia は、Google Skia を CMake ベースで再構成し、C ABI から利用しやすい共有ライブラリ `reskia` として提供することを目指している。
...
`bindings/` は削除済みであり、現行リポジトリは言語別 wrapper を含まない。外部言語からの利用は `skia/capi` と `skia/handles` の C ABI を入口にする。

既存レビューでは `handles` 側の所有権、無効キー処理、世代管理、スレッド安全性に課題が残ると整理されている。
...
- `bindings/` は削除済み。Reskia 本体は CMake build と C ABI の整備に集中する。
...
- 外部 wrapper が安全に利用するには、C ABI 側の所有権規約がまだ足りない。
...
- 外部 wrapper が安全に利用する前提として所有権規約が必要。
- stale handle、無効キー、所有権移動の問題は、機能追加後ほど修正コストが上がる。
- `docs/ja/plans/c-binding-remediation/` に計画とチェックリストが既にある。

次アクション:

1. `docs/ja/plans/c-binding-remediation/checklists` の現状を再確認する。
2. `handles` の `operator[]` 排除、非破壊 borrow、mutex 導入の未完了箇所を洗い出す。
3. `SkRefCnt` 系を `retain/release` 規約へ寄せる。
4. C API smoke で retain / release / invalid handle の最小回帰テストを増やす。
...
### F. 外部言語利用向け C ABI の整理

中優先。言語別 wrapper はリポジトリ外で扱うため、Reskia 側では C ABI の規約と検証を整える。

理由:

- 外部 wrapper が安全に実装できるかは、C 側の所有権規約と invalid input 規約に依存する。
- Reskia 本体に言語別 wrapper を戻さず、C ABI とドキュメントを安定させる方が現在の責務に合う。

次アクション:

1. C ABI の `retain/release/borrow` 規約を header と guide で一貫させる。
2. 外部 wrapper が参照できる minimal examples を C API smoke として維持する。
3. 言語別 wrapper の追加は Reskia 本体の対象外として扱う。
...
2. 外部 wrapper が利用しやすい C ABI 規約とサンプルを整える。
...
次に最も効果が高い作業は、`docs/ja/plans/c-binding-remediation/` に沿って C ABI の所有権・ハンドル安全性を固め、その上で `skparagraph` などの C API を段階的に整えることである。
