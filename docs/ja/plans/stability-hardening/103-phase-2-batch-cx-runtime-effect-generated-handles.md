# 103 Phase 2 Batch CX: runtime effect generated handle ownership

開始時刻: 2026-05-11 06:04:05 JST

## 目的

`102-phase-2-batch-cw-image-color-space-handles.md` の「次バッチ候補」から、`SkRuntimeEffect` makeShader / makeColorFilter が実際に生成した shader / color filter handle の ownership を smoke test で固定する。`SkRuntimeEffect::Result` が保持する有効 effect から生成した handle が registry に存在し、呼び出し側で delete できることを確認する。

## 変更内容

- `skia/test/test_runtime_effect_invalid_input_smoke.cpp`
  - 有効な shader SKSL / color filter SKSL を `SkRuntimeEffect_MakeFor*Default` に渡し、`SkRuntimeEffect::Result::effect` から `makeShader` / `makeColorFilter` を呼ぶ smoke ケースを追加した。
  - 生成された `sk_shader_t` / `sk_color_filter_t` が registry に存在することを確認し、delete できることを固定した。
- `cmake/reskia/tests.cmake`
  - `SkRuntimeEffect::Result` を直接確認する smoke target に限定して `cxx_std_17` を指定した。
- `docs/ja/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_runtime_effect.*` の P2 記録を Batch CX に更新した。
...
- `SkImage` refColorSpace / colorSpace borrowed/retained handle 規約を再確認する。
