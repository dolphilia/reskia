# 97 Phase 2 Batch CR: runtime effect data child inputs

開始時刻: 2026-05-11 05:43:04 JST

## 目的

`96-phase-2-batch-cq-image-read-pixels.md` の「次バッチ候補」から、`SkRuntimeEffect` makeShader / makeColorFilter / makeBlender の data handle 0 と child pointer handle validity を再確認する。data handle 0 は empty data として許可しつつ、非 0 の無効 data handle は Skia に渡らないようにする。child pointer handle 0 は Batch BF の規約どおり empty child list として許可する。

## 変更内容

- `skia/capi/sk_runtime_effect.cpp`
  - makeShader / makeShaderWithChildPtr / makeColorFilter / makeColorFilterWithChildren / makeColorFilterWithChildPtr / makeBlender で、非 0 の無効 data handle を `0` return にした。
- `skia/capi/sk_runtime_effect.h`
  - data handle 0 は empty data として許可、非 0 data は valid handle 必須、child pointer handle 0 は empty input として許可する規約を明記した。
- `skia/test/test_runtime_effect_invalid_input_smoke.cpp`
  - NULL receiver 経路で invalid data / child pointer handle を渡しても `0` return になる smoke ケースを追加した。
- `docs/ja/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_runtime_effect.*` の P2 記録を Batch CR に更新した。
