# 127 Phase 2 Batch DV: image peek and scale pixmap storage

開始時刻: 2026-05-11 10:59:49 JST

## 目的

`126-phase-2-batch-du-canvas-read-pixels-storage.md` の「次バッチ候補」から、`SkImage` peekPixels / scalePixels pixmap storage の required output pointer と false sentinel 規約を再確認する。peekPixels は non-null out pixmap、scalePixels は writable pixmap storage と sampling を必須にし、invalid input を false に固定する。

## 変更内容

- `skia/capi/sk_image.h`
  - `SkImage_peekPixels` の non-null out pixmap / borrowed pixels 規約と、`SkImage_scalePixels` の writable storage required 規約を明記した。
- `skia/test/test_image_surface_invalid_input_smoke.cpp`
  - NULL image + valid pixmap が false になること、raster image + valid pixmap の peekPixels が成功することを確認した。
  - valid writable pixmap storage + sampling の scalePixels 成功 smoke を追加した。
- `docs/ja/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_image.*` の P2 記録を Batch DV に更新した。
