# 124 Phase 2 Batch DS: image readPixels context and rowBytes

開始時刻: 2026-05-11 10:36:07 JST

## 目的

`123-phase-2-batch-dr-canvas-drawable-picture-matrix.md` の「次バッチ候補」から、`SkImage` readPixels pixmap / raw buffer の nullable context と rowBytes 規約を再確認する。context は NULL を許可し、dstInfo / dstPixels / pixmap が必須で、raw buffer 版では rowBytes が `dstInfo.validRowBytes` を満たす場合だけ Skia API に渡す。

## 変更内容

- `skia/capi/sk_image.h`
  - `SkImage_readPixels` / `SkImage_readPixelsWithContextPixmap` の context nullable 規約を明記した。
- `skia/test/test_image_surface_invalid_input_smoke.cpp`
  - NULL context + valid raw buffer / valid rowBytes の成功 smoke を追加した。
  - NULL context + valid pixmap の成功 smoke を追加した。
- `docs/ja/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_image.*` の P2 記録を Batch DS に更新した。
