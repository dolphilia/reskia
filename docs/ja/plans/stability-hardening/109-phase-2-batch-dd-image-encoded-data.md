# 109 Phase 2 Batch DD: image encoded data handle ownership

開始時刻: 2026-05-11 06:22:29 JST

## 目的

`108-phase-2-batch-dc-canvas-color-draw.md` の「次バッチ候補」から、`SkImage_refEncodedData` の null result ownership を再確認する。Skia が encoded data を持たない image で NULL `sk_sp<SkData>` を返した場合に、registry 上の NULL data handle を作らず C API の handle `0` に正規化する。

## 変更内容

- `skia/capi/sk_image.cpp`
  - `make_data_handle` を追加し、`SkImage_refEncodedData` の null result を handle `0` に正規化した。
- `skia/capi/sk_image.h`
  - `SkImage_refEncodedData` の retained handle ownership と、NULL image / no encoded data で `0` を返す規約を明記した。
- `skia/test/test_image_surface_invalid_input_smoke.cpp`
  - raster snapshot image が encoded data を持たない場合に `SkImage_refEncodedData` が `0` を返す smoke ケースを追加した。
- `docs/ja/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_image.*` の P2 記録を Batch DD に更新した。
