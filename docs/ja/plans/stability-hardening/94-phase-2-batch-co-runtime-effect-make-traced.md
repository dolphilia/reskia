# 94 Phase 2 Batch CO: runtime effect traced shader handle safety

開始時刻: 2026-05-11 05:38:11 JST

## 目的

`93-phase-2-batch-cn-image-shader-guards.md` の「次バッチ候補」から、`SkRuntimeEffect_MakeTraced` の shader handle 0 / invalid handle と traceCoord required 規約を確認する。registry に存在しない shader handle が `static_sk_shader_get_entity` に渡らないようにする。

## 変更内容

- `skia/capi/sk_runtime_effect.cpp`
  - `SkRuntimeEffect_MakeTraced` は `shader` が 0、または registry に存在しない handle の場合 `0` を返すようにした。
- `skia/capi/sk_runtime_effect.h`
  - shader valid handle、traceCoord non-null、invalid input returns 0 規約を明記した。
- `skia/test/test_runtime_effect_invalid_input_smoke.cpp`
  - `SkRuntimeEffect_MakeTraced` の shader handle 0、invalid handle、NULL traceCoord smoke ケースを追加した。
- `docs/ja/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_runtime_effect.*` の P2 記録を Batch CO に更新した。
