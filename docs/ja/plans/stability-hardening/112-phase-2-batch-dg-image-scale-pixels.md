# 112 Phase 2 Batch DG: image scalePixels required inputs

開始時刻: 2026-05-11 10:05:48 JST

## 目的

`111-phase-2-batch-df-canvas-annotation.md` の「次バッチ候補」から、`SkImage` scalePixels の required pixmap / sampling 規約を追加確認する。image / dst pixmap / sampling が欠ける場合は false を返す規約を smoke test と公開ヘッダで固定する。

## 変更内容

- `skia/capi/sk_image.h`
  - `SkImage_scalePixels` の image / dst / sampling required と invalid input false 規約を明記した。
- `skia/test/test_image_surface_invalid_input_smoke.cpp`
  - NULL image、NULL pixmap、NULL sampling の smoke ケースを追加した。
- `docs/ja/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_image.h` の P2 記録を Batch DG に更新した。
