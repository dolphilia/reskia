# 80 Phase 2 Batch CA: canvas pixel buffer and rowBytes safety

開始時刻: 2026-05-10 21:42:20 JST

## 目的

`79-phase-2-batch-bz-font-text-encoding.md` の「次バッチ候補」から、`SkCanvas` readPixels / writePixels 周辺の rowBytes / pixel buffer 規約を追加確認する。ImageInfo 版 API で NULL buffer や不正な rowBytes が Skia API に渡らないよう、C API 境界で false に固定する。

## 変更内容

- `skia/capi/sk_canvas.cpp`
  - `SkCanvas_readPixelsWithImageInfo` / `SkCanvas_writePixelsWithImageInfo` は `info` / pixel buffer が NULL、または `rowBytes` が `SkImageInfo::validRowBytes` を満たさない場合 false を返すようにした。
- `skia/capi/sk_canvas.h`
  - caller-owned pixel buffer、rowBytes、invalid input false の規約を明記した。
- `skia/test/test_canvas_invalid_input_smoke.cpp`
  - NULL ImageInfo / NULL pixels / invalid rowBytes の read/write smoke ケースを追加した。
- `docs/ja/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_canvas.*` の P2 記録を Batch CA に更新した。
