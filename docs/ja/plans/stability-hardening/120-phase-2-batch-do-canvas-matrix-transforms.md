# 120 Phase 2 Batch DO: canvas matrix transform rules

開始時刻: 2026-05-11 10:30:44 JST

## 目的

`119-phase-2-batch-dn-runtime-effect-child-ptr-handles.md` の「次バッチ候補」から、`SkCanvas` concat / setMatrix の matrix pointer required と invalid input no-op 規約を再確認する。既存実装の NULL guard を smoke test で固定し、公開ヘッダのコメントを no-op 規約に揃える。

## 変更内容

- `skia/capi/sk_canvas.h`
  - `SkCanvas_concat` / `SkCanvas_concatMatrix` / `SkCanvas_setMatrix` / `SkCanvas_setMatrix3x3` の required matrix pointer と invalid input no-op 規約を明記した。
- `skia/test/test_canvas_invalid_input_smoke.cpp`
  - matrix pointer NULL の no-op と、valid `SkM44` / `SkMatrix` pointer 入力の smoke ケースを追加した。
- `docs/ja/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_canvas.*` の P2 記録を Batch DO に更新した。
