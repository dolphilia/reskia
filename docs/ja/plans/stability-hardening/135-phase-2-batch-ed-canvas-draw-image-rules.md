# 135 Phase 2 Batch ED: canvas drawImage rules

開始時刻: 2026-05-12 23:40:23 JST

## 目的

`134-phase-2-batch-ec-runtime-builder-make-ownership.md` の「次バッチ候補」から、`SkCanvas` drawImage / drawImageRect の image non-null と sampling / paint optional 規約を再確認する。handle 版は valid image handle が必須、sampling 付き overload は sampling が必須、paint は NULL 許可で no-op になることを固定する。

## 変更内容

- `skia/capi/sk_canvas.h`
  - `SkCanvas_drawImage*` / `SkCanvas_drawImageRect*` の required input と invalid input no-op 規約を明記した。
  - sampling 付き overload では sampling が必須で、paint は optional であることを明確にした。
- `skia/test/test_canvas_invalid_input_smoke.cpp`
  - valid image handle と NULL paint で `SkCanvas_drawImage` / `SkCanvas_drawImageHandleWithSampling` / `SkCanvas_drawImageRect` / `SkCanvas_drawImageRectHandleWithSrcDst` が呼べることを確認した。
  - valid image handle でも NULL sampling の sampling 付き overload が no-op でクラッシュしないことを確認した。
- `docs/ja/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_canvas.cpp` / `sk_canvas.h` の P2 記録を Batch ED に更新した。
