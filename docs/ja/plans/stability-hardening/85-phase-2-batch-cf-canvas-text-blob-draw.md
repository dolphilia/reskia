# 85 Phase 2 Batch CF: canvas text blob draw safety

開始時刻: 2026-05-11 00:32:52 JST

## 目的

`84-phase-2-batch-ce-surface-pixels-rowbytes.md` の「次バッチ候補」から、`SkCanvas` drawTextBlob handle / pointer API の handle 0、borrowed pointer、paint 必須規約を追加確認する。handle 版では非 0 でも registry に存在しない handle が Skia に渡らないよう、C API 境界で no-op に固定する。

## 変更内容

- `skia/capi/sk_canvas.cpp`
  - `SkCanvas_drawTextBlob` は `text_blob` が 0、または registry に存在しない handle の場合 no-op にした。
- `skia/capi/sk_canvas.h`
  - `SkCanvas_drawTextBlob` は valid handle と non-null paint が必須で、invalid input は no-op と明記した。
  - `SkCanvas_drawTextBlobPtr` は borrowed non-null blob と non-null paint が必須で、invalid input は no-op と明記した。
- `skia/test/test_canvas_invalid_input_smoke.cpp`
  - handle 0、無効 handle、NULL blob pointer、NULL paint、valid handle / pointer の drawTextBlob smoke ケースを追加した。
- `docs/ja/plans/c-binding-remediation/checklists/capi-status.csv`
  - `sk_canvas.*` の P2 記録を Batch CF に更新した。
