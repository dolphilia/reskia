...
`skia/capi` と `skia/handles` の設計改善を、外部言語から利用しやすい C ABI として段階実施する計画セットです。
...
- 外部 wrapper からも安全に運用できる C API を整備する。
...
4. `05-phase-4-migration-and-cleanup.md` を完了（移行/整理）
5. `06-public-api-coverage-roadmap.md` に従い、公開 API coverage を優先度順に改善

## 廃止済み計画

- `04-phase-3-rust-ffi-rollout.md`: 2026-06-02 に `bindings/` と root Rust workspace を削除したため、現行計画から外す。C ABI 側の所有権・lifetime 規約は引き続き本計画内で扱う。
