...
ベースSkia: `vendor/skia-source.lock` の `SKIA_REF` を正とする。

更新メモ（2026-06-02）:

- 現在の Skia baseline は `70f9d90bc8e6a56101d036153cfef28088e57f5b`（baseline date `2026-06-01`）。
- C API coverage は `missing 0` / `deferred 0` / `partial 0` / `overcovered 0` を維持している。
- `vendor/skia-upstream` の段階的アップグレードは、固定 commit ごとの継続サイクルとして main 上で運用する。
- 継続計画、引き継ぎ、cycle 記録は `docs/ja/plans/skia-incremental-upgrade/` に置く。
- cycle 091 で local refs 上の `70f9d90bc8e6a56101d036153cfef28088e57f5b` まで追従済み。
...
- `skia/modules/skpath/`: `skpath_skcms` 互換ターゲット
- `skia/modules/skresources/`: `skresources` 静的ライブラリ
- `skia/modules/svg/`: `svg`, `skshaper`, `skunicode` 静的ライブラリ
- `bindings/`: 外部言語 binding 用の補助領域
- `cmake/`: 依存解決や CMake helper
- `third_party/`: dependency source/build/install 管理
- `vendor/`: Skia 参照 checkout と lock

ファイル規模の数値は Skia 追従サイクルごとに変わるため、この文書では固定値を持たない。
...
- `skia/modules/skcms/CMakeLists.txt`: `add_library(skcms ...)`
- `skia/modules/skresources/CMakeLists.txt`: `add_library(skresources ...)`
- `skia/modules/svg/CMakeLists.txt`:
  - `add_library(svg ...)`
  - `add_library(skshaper ...)`
  - `add_library(skunicode ...)`
- `skia/modules/skpath/CMakeLists.txt`:
  - `skpath_skcms` 互換ターゲットを提供する
...
今後の C API 追加・レビューでは、`vendor/skia-upstream` の公開ヘッダと `skia/capi` / `skia/handles` の対応状況を確認し、対象外とする API には platform dependency、optional module、所有権、ABI 安全性などの理由を残す。method-level の進捗確認には `scripts/generate_public_api_coverage.py` と `docs/ja/plans/c-binding-remediation/checklists/public-api-coverage-matrix.csv` を使う。
...
- `/Users/dolphilia/github/reskia/skia/test/`
- `/Users/dolphilia/github/reskia/skia/modules/skcms/CMakeLists.txt`
- `/Users/dolphilia/github/reskia/skia/modules/skpath/CMakeLists.txt`
- `/Users/dolphilia/github/reskia/skia/modules/skresources/CMakeLists.txt`
- `/Users/dolphilia/github/reskia/skia/modules/svg/CMakeLists.txt`
...
詳細は以下を参照:
- `/Users/dolphilia/github/reskia/docs/ja/notes/skia-cmakelists-build-report-2026-02-13.md`

## 9. 非`skia` CMake修正メモ（2026-02-14）

当時は `skia` 以外のCMakeプロジェクト（`skcms`, `skpath`, `skresources`, `svg`）を同様に検証した。現在これらは `skia/modules/*` 配下に集約されている。

### 9.1 `skpath` の修正

対象:
- 現在: `/Users/dolphilia/github/reskia/skia/modules/skpath/CMakeLists.txt`
- 当時: `/Users/dolphilia/github/reskia/skpath/CMakeLists.txt`
...
対象:
- 現在: `/Users/dolphilia/github/reskia/skia/modules/svg/CMakeLists.txt`
- 当時: `/Users/dolphilia/github/reskia/svg/CMakeLists.txt`
...
詳細ログは以下を参照:
- `/Users/dolphilia/github/reskia/docs/ja/notes/dependency-management-report-2026-02-14.md`
...
詳細ログ:
- `/Users/dolphilia/github/reskia/docs/ja/notes/third-party-submodule-automation-2026-02-14.md`
...
詳細:
- `/Users/dolphilia/github/reskia/docs/ja/notes/c-binding-static-binding-review-2026-02-14.md`
...
- `skia/modules/svg/CMakeLists.txt` で `svg` / `skshaper` / `skunicode` をまとめて扱っており、依存検出と target 境界が複雑
- `skia/modules/skpath` は `skpath_skcms` 互換ターゲットとして残っており、役割を明示する必要がある
...
- `skia/modules/svg` と `skia/modules/skpath` の命名・構造を明確化
- 将来的にルート superbuild を導入し、モジュールと依存モードを一元管理

詳細:
- `/Users/dolphilia/github/reskia/docs/ja/notes/cmake-architecture-review-2026-02-14.md`
...
詳細:
- `/Users/dolphilia/github/reskia/docs/ja/notes/reskia-feature-gap-survey-2026-02-14.md`

## 16. Public API coverage freeze と段階的アップグレード（2026-06-02）

公開 API binding 拡張作業により、method-level coverage matrix は `missing 0` / `deferred 0` / `partial 0` / `overcovered 0` まで閉じた。

この状態を今後の Skia 追従の gate として使う。`vendor/skia-upstream` を新しい commit へ進める場合は、直接 latest main へ飛ばず、固定 commit を候補にして probe する。

継続運用:

- 作業ブランチ: `main`
- 現在の Skia baseline: `70f9d90bc8e6a56101d036153cfef28088e57f5b`
- 直近完了サイクル: cycle 091
- 計画: `/Users/dolphilia/github/reskia/docs/ja/plans/skia-incremental-upgrade/00-continuous-upgrade-cycle.md`
- 引き継ぎ: `/Users/dolphilia/github/reskia/docs/ja/plans/skia-incremental-upgrade/HANDOFF.md`
- 記録先: `/Users/dolphilia/github/reskia/docs/ja/plans/skia-incremental-upgrade/records/`
- 調査メモ: `/Users/dolphilia/github/reskia/docs/ja/notes/skia-incremental-upgrade-readiness-2026-05-22.md`

初回候補 `5f54e9f84cff8c42fd645ec53c1727857bdb12ab` から始まったサイクルは cycle 091 まで進み、現在は `70f9d90bc8e6a56101d036153cfef28088e57f5b` を accepted baseline としている。次サイクルでは、この baseline 後の固定 local ref を再確認する。
