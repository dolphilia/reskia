...
`docs/ja/plans/stability-hardening/` の各バッチ記録は実施ログであり、この文書は今後の編集時に参照する作業標準である。
...
候補は 5 件程度にし、それぞれが 1 バッチとして実行できる粒度にする。
...
- `capi-status.csv`、`capi-safety-detail-status.csv`、バッチ記録。
- `git diff --check`。

## Checklist 更新

`docs/ja/plans/c-binding-remediation/checklists/capi-status.csv` は、対象ファイル単位で更新する。
...
`docs/ja/plans/c-binding-remediation/checklists/capi-safety-detail-status.csv` は、API 群と安全性規約単位で更新する。これは `capi-status.csv` より細かい重複防止用の台帳であり、同じファイル内の別 API 群を何度も再確認しないための一次参照にする。

バッチ開始前に必ず `capi-safety-detail-status.csv` を検索し、同じ `area` / `api_pattern` / `rule_category` が既に `verified` / `documented` / `covered` になっていないか確認する。既に確認済みの場合は、同じ作業を繰り返さず、足りない列だけを補う。

更新する内容:

- `id`: 安定した snake_case の API 群 ID
- `area`: `SkCanvas` などの対象クラスまたは API 群
- `api_pattern`: 対象関数名または `|` 区切りの関数群
- `files`: 対象ファイルを `;` 区切りで記録
- `rule_category`: `borrowed-pointer`、`pixel-storage-rowbytes`、`callback-failure` などの規約分類
- `receiver_rule` / `input_rules` / `output_or_lifetime_rule` / `invalid_behavior`: ガイド上の規約を短く記録
...
- `valid_path_status`: `covered` / `representative-valid-path` / `omitted-with-reason`
- `last_batch` / `last_checked_at` / `evidence`: batch 名、日付、該当バッチ記録
- `notes`: 省略理由、feature-gated 条件、公開 API が存在しない場合の理由

1 バッチで複数の規約単位を扱う場合は、規約単位ごとに 1 行ずつ追加または更新する。既存行を更新する場合は、古い batch 名を消すのではなく `notes` に重要な例外を残し、`last_batch` を最新 batch にする。

## バッチ記録

`docs/ja/plans/stability-hardening/NNN-phase-2-batch-xx-*.md` は次の形にする。
...
- `docs/ja/plans/c-binding-remediation/checklists/capi-safety-detail-status.csv`
  - 追加または更新した detail row の `id` を記録する。
...
`capi-safety-detail-status.csv` を更新した場合は、次も実行する。
...
- `capi-safety-detail-status.csv` に、今回確認した API 群と規約分類が具体的に残っている。
...
- `docs/ja/plans/c-binding-remediation/00-objectives-and-constraints.md`
- `docs/ja/plans/c-binding-remediation/03-phase-2-ownership-api.md`
