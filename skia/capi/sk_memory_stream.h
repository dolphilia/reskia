...
reskia_memory_stream_t *SkMemoryStream_newWithDataAndCopyFlag(const uint8_t *data, size_t length, bool copyData); // data may be null only when length == 0; copyData false borrows caller storage
...
void SkMemoryStream_setMemory(reskia_memory_stream_t *memory_stream, const uint8_t *data, size_t length, bool copyData); // data may be null only when length == 0; copyData false borrows caller storage
...
const uint8_t * SkMemoryStream_getAtPos(reskia_memory_stream_t *memory_stream); // borrowed pointer at current position; caller must not free
size_t SkMemoryStream_read(reskia_memory_stream_t *memory_stream, uint8_t *buffer, size_t size); // buffer may be null to skip size bytes
bool SkMemoryStream_isAtEnd(reskia_memory_stream_t *memory_stream); // (SkMemoryStream *memory_stream) -> bool
size_t SkMemoryStream_peek(reskia_memory_stream_t *memory_stream, uint8_t *buffer, size_t size); // buffer may be null only when size == 0
...
const uint8_t * SkMemoryStream_getMemoryBase(reskia_memory_stream_t *memory_stream); // borrowed memory base; caller must not free
...
sk_memory_stream_t SkMemoryStream_MakeCopy(const uint8_t *data, size_t length); // data may be null only when length == 0; contents are copied
sk_memory_stream_t SkMemoryStream_MakeDirect(const uint8_t *data, size_t length); // data may be null only when length == 0; caller keeps storage alive
