...
/**
 * out: color/mode は NULL 許可。color_filter NULL では false (SkColorFilter *color_filter, SkColor *color, SkBlendMode *mode) -> bool
 */
bool SkColorFilter_asAColorMode(reskia_color_filter_t *color_filter, reskia_color_t *color, int *mode);
/**
 * out: matrix は 20 要素以上、非 NULL。NULL 入力では false (SkColorFilter *color_filter, float matrix[20]) -> bool
 */
...
/**
 * write_buffer は非 NULL。NULL 入力では no-op (SkColorFilter *color_filter, SkWriteBuffer *write_buffer)
 */
...
/**
 * retained data handle。serial_procs は NULL 許可。color_filter NULL では 0 (SkColorFilter *color_filter, const SkSerialProcs *serial_procs) -> sk_data_t
 */
sk_data_t SkColorFilter_serialize(reskia_color_filter_t *color_filter, const reskia_serial_procs_t *serial_procs);
/**
 * raw out buffer: memory は memory_size バイト以上。サイズ取得は serialize() を使う。color_filter NULL では 0
 */
