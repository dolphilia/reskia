...
void SkColorMatrix_delete(reskia_color_matrix_t *color_matrix); // No-op for NULL input. (SkColorMatrix *color_matrix)
void SkColorMatrix_setIdentity(reskia_color_matrix_t *color_matrix); // No-op for NULL input. (SkColorMatrix *color_matrix)
/**
 * No-op for NULL input. (SkColorMatrix *color_matrix, float rScale, float gScale, float bScale, float aScale)
 */
void SkColorMatrix_setScale(reskia_color_matrix_t *color_matrix, float rScale, float gScale, float bScale, float aScale);
/**
 * No-op for NULL input. (SkColorMatrix *color_matrix, float dr, float dg, float db, float da)
 */
void SkColorMatrix_postTranslate(reskia_color_matrix_t *color_matrix, float dr, float dg, float db, float da);
/**
 * color_matrix, a, and b must be non-NULL. No-op for NULL input.
 * (SkColorMatrix *color_matrix, const SkColorMatrix *a, const SkColorMatrix *b)
 */
...
/**
 * color_matrix and mat must be non-NULL. No-op for NULL input.
 * (SkColorMatrix *color_matrix, const SkColorMatrix *mat)
 */
...
void SkColorMatrix_setSaturation(reskia_color_matrix_t *color_matrix, float sat); // No-op for NULL input. (SkColorMatrix *color_matrix, float sat)
void SkColorMatrix_setRowMajor(reskia_color_matrix_t *color_matrix, const float src[20]); // raw input array: src must contain at least 20 elements and be non-NULL. No-op for NULL input.
void SkColorMatrix_getRowMajor(reskia_color_matrix_t *color_matrix, float dst[20]); // raw output array: dst must contain at least 20 elements and be non-NULL. No-op for NULL input.
