...
void SkReadBuffer_setTypefaceArray(reskia_read_buffer_t *buffer, void * array, int count); // (SkReadBuffer* buffer, sk_sp<SkTypeface>* array, int count). count > 0 requires array; invalid input is no-op.
void SkReadBuffer_setFactoryPlayback(reskia_read_buffer_t *buffer, void * array, int count); // (SkReadBuffer* buffer, SkFlattenable::Factory* array, int count). count > 0 requires array; invalid input is no-op.
void SkReadBuffer_setDeserialProcs(reskia_read_buffer_t *buffer, const reskia_deserial_procs_t *procs); // (SkReadBuffer* buffer, const SkDeserialProcs* procs). procs is required.
