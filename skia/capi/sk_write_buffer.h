...
/**
 * (SkWriteBuffer* writeBuffer, const void* buffer, size_t bytes). bytes > 0 requires buffer.
 * invalid input is no-op.
 */
void SkWriteBuffer_writePad32(reskia_write_buffer_t *writeBuffer, const void *buffer, size_t bytes);
/**
 * (SkWriteBuffer* writeBuffer, const void* data, size_t size). size > 0 requires data.
 * invalid input is no-op.
 */
void SkWriteBuffer_writeByteArray(reskia_write_buffer_t *writeBuffer, const void *data, size_t size);
/**
 * (SkWriteBuffer* writeBuffer, const SkData* data). data may be NULL.
 * NULL writeBuffer is no-op.
 */
...
/**
 * (SkWriteBuffer* writeBuffer, const SkScalar* value, uint32_t count). count > 0 requires value.
 * invalid input is no-op.
 */
...
/**
 * (SkWriteBuffer* writeBuffer, const int32_t* value, uint32_t count). count > 0 requires value.
 * invalid input is no-op.
 */
...
/**
 * (SkWriteBuffer* writeBuffer, const SkImage* image). image may be NULL.
 * NULL writeBuffer is no-op.
 */
void SkWriteBuffer_writeImage(reskia_write_buffer_t *writeBuffer, const reskia_image_t *image);
/**
 * (SkWriteBuffer* writeBuffer, SkTypeface* typeface). typeface may be NULL.
 * NULL writeBuffer is no-op.
 */
void SkWriteBuffer_writeTypeface(reskia_write_buffer_t *writeBuffer, reskia_typeface_t *typeface);
/**
 * (SkWriteBuffer* writeBuffer, const SkPaint& paint). paint is required.
 * invalid input is no-op.
 */
