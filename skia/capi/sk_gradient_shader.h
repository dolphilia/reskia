...
/*
 * All gradient factories require non-null geometry pointers for the overload,
 * non-null colors, count > 0, and valid SkTileMode values when a mode is
 * exposed. localMatrix and pos may be NULL. Interpolation overloads require a
 * non-null interpolation pointer. Invalid input or Skia factory failure returns
 * 0; successful shader handles are caller-owned.
 */
