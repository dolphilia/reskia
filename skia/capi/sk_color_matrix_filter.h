...
void SkColorMatrixFilter_release(reskia_color_matrix_filter_t *color_matrix_filter); // owned: caller が保持する参照を release する。NULL 入力では no-op (SkColorMatrixFilter *color_matrix_filter)
bool SkColorMatrixFilter_asAColorMode(reskia_color_matrix_filter_t *color_matrix_filter, reskia_color_t *color, int *mode); // out: color/mode は NULL 許可。color_matrix_filter NULL では false
...
reskia_color_t SkColorMatrixFilter_filterColor(reskia_color_matrix_filter_t *color_matrix_filter, reskia_color_t color); // color_matrix_filter NULL では入力 color を返す
sk_color_4f_t SkColorMatrixFilter_filterColor4f(reskia_color_matrix_filter_t *color_matrix_filter, const reskia_color_4f_t *srcColor, reskia_color_space_t *srcCS, reskia_color_space_t *dstCS); // srcColor は非 NULL。srcCS/dstCS は NULL 許可。NULL 入力では 0
...
void SkColorMatrixFilter_flatten(reskia_color_matrix_filter_t *color_matrix_filter, reskia_write_buffer_t *write_buffer); // write_buffer は非 NULL。NULL 入力では no-op
...
sk_data_t SkColorMatrixFilter_serialize(reskia_color_matrix_filter_t *color_matrix_filter, const reskia_serial_procs_t *serial_procs); // retained data handle。serial_procs は NULL 許可。color_matrix_filter NULL では 0
size_t SkColorMatrixFilter_serializeToMemory(reskia_color_matrix_filter_t *color_matrix_filter, void *memory, size_t memory_size, const reskia_serial_procs_t *serial_procs); // raw out buffer: memory は memory_size バイト以上。サイズ取得は serialize() を使う。NULL 入力では 0
...
sk_color_filter_t SkColorMatrixFilter_Deserialize(const void *data, size_t size, const reskia_deserial_procs_t *procs); // raw input buffer: data は size バイト以上、非 NULL。procs は NULL 許可。生成不能なら 0
