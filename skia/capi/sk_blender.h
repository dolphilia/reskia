...
/**
 * blender/write_buffer は非 NULL。NULL 入力は no-op (SkBlender *blender, SkWriteBuffer *write_buffer)
 */
...
/**
 * serial_procs は NULL 許可。NULL blender または生成不能なら 0 (SkBlender *blender, const SkSerialProcs *serial_procs) -> sk_data_t
 */
sk_data_t SkBlender_serialize(reskia_blender_t *blender, const reskia_serial_procs_t *serial_procs);
/**
 * serial_procs は NULL 許可。NULL blender または memory NULL かつ size 非 0 なら 0 (SkBlender *blender, void *memory, size_t memory_size, const SkSerialProcs *serial_procs) -> size_t
 */
...
/**
 * name/factory は非 NULL。invalid 入力は no-op (const char *name, sk_flattenable_factory_t factory)
 */
void SkBlender_Register(const char * name, sk_flattenable_factory_t factory);
/**
 * procs は NULL 許可。NULL data または length 0 または復元不能なら 0 (SkBlender::Type type, const void *data, size_t length, const SkDeserialProcs *procs) -> sk_flattenable_t
 */
