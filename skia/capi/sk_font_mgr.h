...
reskia_status_t SkFontMgr_createStyleSet(reskia_font_mgr_t *font_mgr, int index, sk_font_style_set_t *out_style_set); // index 範囲内、out_style_set 非 NULL。invalid 入力では out 0 + INVALID_ARGUMENT
reskia_status_t SkFontMgr_matchFamily(reskia_font_mgr_t *font_mgr, const char familyName[], sk_font_style_set_t *out_style_set); // familyName は NULL 許可、out_style_set 非 NULL。生成不能では out 0 + NOT_FOUND
sk_typeface_t SkFontMgr_matchFamilyStyle(reskia_font_mgr_t *font_mgr, const char familyName[], const reskia_font_style_t *font_style); // familyName は NULL 許可、font_style 非 NULL。invalid 入力や生成不能では 0
sk_typeface_t SkFontMgr_matchFamilyStyleCharacter(reskia_font_mgr_t *font_mgr, const char familyName[], const reskia_font_style_t *font_style, const char *bcp47[], int bcp47Count, reskia_font_mgr_unichar_t character); // bcp47Count > 0 では bcp47 非 NULL。invalid 入力や生成不能では 0
...
sk_typeface_t SkFontMgr_makeFromStreamWithArguments(reskia_font_mgr_t *font_mgr, sk_stream_asset_t stream_asset, const reskia_font_arguments_t *font_argments); // stream_asset は consumed、font_arguments 非 NULL。invalid 入力や生成不能では 0
...
sk_typeface_t SkFontMgr_legacyMakeTypeface(reskia_font_mgr_t *font_mgr, const char familyName[], sk_font_style_t style); // familyName は NULL 許可。invalid 入力や生成不能では 0
