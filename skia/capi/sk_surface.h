 ...
 * returned pixmap borrows surface pixels and is valid only while surface storage remains unchanged.
 * invalid input returns false (SkSurface *surface, SkPixmap *pixmap) -> bool.
 */
bool SkSurface_peekPixels(reskia_surface_t *surface, reskia_pixmap_t *pixmap);
/**
 * dst: non-null pixmap.
 * invalid input returns false (SkSurface *surface, const SkPixmap *dst, int srcX, int srcY) -> bool.
 */
 ...
 * invalid input returns false (SkSurface *surface, const SkImageInfo *dstInfo, void *dstPixels, size_t dstRowBytes, int srcX, int srcY) -> bool.
 */
...
/**
 * dst: non-null bitmap.
 * invalid input returns false (SkSurface *surface, const SkBitmap *dst, int srcX, int srcY) -> bool.
 */
bool SkSurface_readPixelsWithBitmap(reskia_surface_t *surface, const reskia_bitmap_t *dst, int srcX, int srcY);

/**
 * surface/info/srcRect/callback are required.
 * invalid input invokes callback with NULL result when callback is non-null (SkSurface *surface, const SkImageInfo *info, const SkIRect *srcRect, SkSurface::RescaleGamma rescaleGamma, SkSurface::RescaleMode rescaleMode, reskia_async_read_pixels_callback_t callback, void *context)
 */
void SkSurface_asyncRescaleAndReadPixels(reskia_surface_t *surface, const reskia_image_info_t *info, const reskia_i_rect_t *srcRect, reskia_surface_rescale_gamma_t rescaleGamma, reskia_surface_rescale_mode_t rescaleMode, reskia_async_read_pixels_callback_t callback, void *context);
/**
 * surface/srcRect/callback and valid dstSize handle are required.
 * invalid input invokes callback with NULL result when callback is non-null (SkSurface *surface, sk_color_space_t dstColorSpace, SkYUVColorSpace yuvColorSpace, const SkIRect *srcRect, sk_i_size_t dstSize, SkSurface::RescaleGamma rescaleGamma, SkSurface::RescaleMode rescaleMode, reskia_async_read_pixels_callback_t callback, void *context)
 */
...
/**
 * src: non-null pixmap.
 * invalid input is no-op (SkSurface *surface, const SkPixmap *src, int dstX, int dstY)
 */
