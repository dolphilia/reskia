...
/**
 * dimensions は非 NULL。matrix/paint は NULL 許可。invalid picture/props は empty 扱い、生成不能なら 0 (sk_picture_t picture, const SkISize *dimensions, const SkMatrix *matrix, const SkPaint *paint, SkImages::BitDepth bitDepth, sk_color_space_t color_space, sk_surface_props_t props) -> sk_image_t
 */
...
/**
 * dimensions は非 NULL。matrix/paint は NULL 許可。invalid picture/生成不能なら 0 (sk_picture_t picture, const SkISize *dimensions, const SkMatrix *matrix, const SkPaint *paint, SkImages::BitDepth bitDepth, sk_color_space_t color_space) -> sk_image_t
 */
...
/**
 * pixmap は非 NULL。release proc/context は NULL 許可。生成不能なら 0 (const SkPixmap *pixmap, SkImages::RasterReleaseProc rasterReleaseProc, SkImages::ReleaseContext releaseContext) -> sk_image_t
 */
...
/**
 * info は非 NULL。invalid data/生成不能なら 0 (const SkImageInfo *info, sk_data_t data, size_t rowBytes) -> sk_image_t
 */
sk_image_t SkImages_RasterFromData(const reskia_image_info_t *info, sk_data_t data, size_t rowBytes);
/**
 * subset/clip_bounds/out_subset/out_offset/out_image は非 NULL。失敗時 out_image は 0
 */
