...
void SkColor_SkColorToHSV(reskia_color_t color, float hsv[3]); // raw output array: hsv must contain at least 3 elements and be non-NULL. No-op for NULL input. (SkColor color, SkScalar hsv[3])
reskia_color_t SkColor_SkHSVToColor(const float hsv[3]); // raw input array: hsv must contain at least 3 elements and be non-NULL. Returns 0 for NULL input. (const SkScalar hsv[3]) -> SkColor
/**
 * raw input array: hsv must contain at least 3 elements and be non-NULL. Returns 0 for NULL input.
 * (U8CPU alpha, const SkScalar hsv[3]) -> SkColor
 */
...
/**
 * raw output array: hsv must contain at least 3 elements and be non-NULL. No-op for NULL input.
 * (U8CPU red, U8CPU green, U8CPU blue, SkScalar hsv[3])
 */
