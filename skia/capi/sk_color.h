...
reskia_color_t SkColor_SkHSVToColorWithAlpha(reskia_u32_t alpha, const float hsv[3]); // raw input array: hsv は 3 要素以上、非 NULL。NULL 入力では 0 (U8CPU alpha, const SkScalar hsv[3]) -> SkColor
...
void SkColor_SkRGBToHSV(reskia_u32_t red, reskia_u32_t green, reskia_u32_t blue, float hsv[3]); // raw output array: hsv は 3 要素以上、非 NULL。NULL 入力では no-op (U8CPU red, U8CPU green, U8CPU blue, SkScalar hsv[3])
