...
void SkColorSpace_toProfile(reskia_color_space_t *color_space, reskia_icc_profile_t *profile); // out: profile は非 NULL。NULL 入力では no-op (SkColorSpace *color_space, skcms_ICCProfile *profile)
...
bool SkColorSpace_isNumericalTransferFn(reskia_color_space_t *color_space, reskia_transfer_function_t *fn); // out: fn は非 NULL。NULL 入力では false (SkColorSpace *color_space, skcms_TransferFunction *fn) -> bool
bool SkColorSpace_toXYZD50(reskia_color_space_t *color_space, reskia_matrix3x3_t *toXYZD50); // out: toXYZD50 は非 NULL。NULL 入力では false (SkColorSpace *color_space, skcms_Matrix3x3 *toXYZD50) -> bool
...
sk_data_t SkColorSpace_serialize(reskia_color_space_t *color_space); // retained data handle: 呼び出し側が static_sk_data_delete する。NULL 入力では 0 (SkColorSpace *color_space) -> sk_data_t
size_t SkColorSpace_writeToMemory(reskia_color_space_t *color_space, void *memory); // raw out buffer: memory が NULL なら必要サイズだけ返す。color_space NULL では 0 (SkColorSpace *color_space, void *memory) -> size_t
void SkColorSpace_transferFn(reskia_color_space_t *color_space, float gabcdef[7]); // out: gabcdef は 7 要素以上、非 NULL。NULL 入力では no-op (SkColorSpace *color_space, float gabcdef[7])
void SkColorSpace_transferFnToStruct(reskia_color_space_t *color_space, reskia_transfer_function_t *fn); // out: fn は非 NULL。NULL 入力では no-op (SkColorSpace *color_space, skcms_TransferFunction *fn)
void SkColorSpace_invTransferFn(reskia_color_space_t *color_space, reskia_transfer_function_t *fn); // out: fn は非 NULL。NULL 入力では no-op (SkColorSpace *color_space, skcms_TransferFunction *fn)
void SkColorSpace_gamutTransformTo(reskia_color_space_t *color_space, const reskia_color_space_t *dst, reskia_matrix3x3_t *src_to_dst); // out: src_to_dst は非 NULL。dst は NULL 許可。NULL 入力では no-op (SkColorSpace *color_space, const SkColorSpace *dst, skcms_Matrix3x3 *src_to_dst)
...
sk_color_space_t SkColorSpace_MakeRGB(const reskia_transfer_function_t *transferFn, const reskia_matrix3x3_t *toXYZ); // transferFn/toXYZ は非 NULL。NULL 入力では 0 (const skcms_TransferFunction *transferFn, const skcms_Matrix3x3 *toXYZ) -> sk_color_space_t
...
sk_color_space_t SkColorSpace_Deserialize(const void *data, size_t length); // raw input buffer: data は length バイト以上、非 NULL。NULL 入力では 0 (const void *data, size_t length) -> sk_color_space_t
