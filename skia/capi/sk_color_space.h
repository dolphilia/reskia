...
/**
 * out: profile は非 NULL。NULL 入力では no-op (SkColorSpace *color_space, skcms_ICCProfile *profile)
 */
...
/**
 * out: fn は非 NULL。NULL 入力では false (SkColorSpace *color_space, skcms_TransferFunction *fn) -> bool
 */
bool SkColorSpace_isNumericalTransferFn(reskia_color_space_t *color_space, reskia_transfer_function_t *fn);
/**
 * out: toXYZD50 は非 NULL。NULL 入力では false (SkColorSpace *color_space, skcms_Matrix3x3 *toXYZD50) -> bool
 */
...
/**
 * retained data handle: 呼び出し側が static_sk_data_delete する。NULL 入力では 0 (SkColorSpace *color_space) -> sk_data_t
 */
sk_data_t SkColorSpace_serialize(reskia_color_space_t *color_space);
/**
 * raw out buffer: memory が NULL なら必要サイズだけ返す。color_space NULL では 0 (SkColorSpace *color_space, void *memory) -> size_t
 */
size_t SkColorSpace_writeToMemory(reskia_color_space_t *color_space, void *memory);
/**
 * out: gabcdef は 7 要素以上、非 NULL。NULL 入力では no-op (SkColorSpace *color_space, float gabcdef[7])
 */
void SkColorSpace_transferFn(reskia_color_space_t *color_space, float gabcdef[7]);
/**
 * out: fn は非 NULL。NULL 入力では no-op (SkColorSpace *color_space, skcms_TransferFunction *fn)
 */
...
/**
 * out: src_to_dst は非 NULL。dst は NULL 許可。NULL 入力では no-op (SkColorSpace *color_space, const SkColorSpace *dst, skcms_Matrix3x3 *src_to_dst)
 */
...
/**
 * transferFn/toXYZ は非 NULL。NULL 入力では 0 (const skcms_TransferFunction *transferFn, const skcms_Matrix3x3 *toXYZ) -> sk_color_space_t
 */
