...
sk_color_filter_t SkColorFilters_Blend(const reskia_color_4f_t *c, sk_color_space_t color_space, reskia_blend_mode_t mode); // c は非 NULL。color_space 0/無効は sRGB 扱い。生成不能なら 0 (const SkColor4f *c, sk_color_space_t color_space, SkBlendMode mode) -> sk_color_filter_t
...
sk_color_filter_t SkColorFilters_HSLAMatrix(const float rowMajor[20]); // raw input array: rowMajor は 20 要素以上、非 NULL。生成不能なら 0 (const float rowMajor[20]) -> sk_color_filter_t
sk_color_filter_t SkColorFilters_HSLAMatrixFromMatrix(const reskia_color_matrix_t *matrix); // matrix は非 NULL。生成不能なら 0 (const SkColorMatrix *matrix) -> sk_color_filter_t
...
sk_color_filter_t SkColorFilters_Matrix(const float rowMajor[20]); // raw input array: rowMajor は 20 要素以上、非 NULL。生成不能なら 0 (const float rowMajor[20]) -> sk_color_filter_t
sk_color_filter_t SkColorFilters_MatrixFromMatrix(const reskia_color_matrix_t *matrix); // matrix は非 NULL。生成不能なら 0 (const SkColorMatrix *matrix) -> sk_color_filter_t
sk_color_filter_t SkColorFilters_SRGBToLinearGamma(); // () -> sk_color_filter_t
sk_color_filter_t SkColorFilters_Table(const uint8_t table[256]); // raw input array: table は 256 要素以上、非 NULL。生成不能なら 0 (const uint8_t table[256]) -> sk_color_filter_t
...
sk_color_filter_t SkColorFilters_TableARGB(const uint8_t tableA[256], const uint8_t tableR[256], const uint8_t tableG[256], const uint8_t tableB[256]); // 各 table は NULL 許可で identity 扱い (const uint8_t tableA[256], const uint8_t tableR[256], const uint8_t tableG[256], const uint8_t tableB[256]) -> sk_color_filter_t
