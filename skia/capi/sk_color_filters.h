...
/**
 * c must be non-NULL. color_space 0/invalid is treated as sRGB. Returns 0 on failure.
 * (const SkColor4f *c, sk_color_space_t color_space, SkBlendMode mode) -> sk_color_filter_t
 */
...
sk_color_filter_t SkColorFilters_Compose(sk_color_filter_t color_filter1, sk_color_filter_t color_filter2); // handle 0/invalid means null filter. Returns 0 on failure. (sk_color_filter_t color_filter1, sk_color_filter_t color_filter2) -> sk_color_filter_t
/**
 * raw input array: rowMajor must contain at least 20 elements and be non-NULL. Returns 0 on failure.
 * (const float rowMajor[20]) -> sk_color_filter_t
 */
sk_color_filter_t SkColorFilters_HSLAMatrix(const float rowMajor[20]);
/**
 * matrix must be non-NULL. Returns 0 on failure. (const SkColorMatrix *matrix) -> sk_color_filter_t
 */
sk_color_filter_t SkColorFilters_HSLAMatrixFromMatrix(const reskia_color_matrix_t *matrix);
sk_color_filter_t SkColorFilters_Lerp(float t, sk_color_filter_t color_filter1, sk_color_filter_t color_filter2); // handle 0/invalid means null filter. Returns 0 on failure. (float t, sk_color_filter_t color_filter1, sk_color_filter_t color_filter2) -> sk_color_filter_t
...
/**
 * raw input array: table must contain at least 256 elements and be non-NULL. Returns 0 on failure.
 * (const uint8_t table[256]) -> sk_color_filter_t
 */
sk_color_filter_t SkColorFilters_Table(const uint8_t table[256]);
sk_color_filter_t SkColorFilters_TableFromColorTable(sk_color_table_t color_table); // color_table 0/invalid means null table. Returns 0 on failure. (sk_color_table_t color_table) -> sk_color_filter_t
/**
 * Each table may be NULL and is treated as identity.
 * (const uint8_t tableA[256], const uint8_t tableR[256], const uint8_t tableG[256], const uint8_t tableB[256]) -> sk_color_filter_t
 */
