...
int SkPicture_cullRect(reskia_picture_t *picture); // (SkPicture *picture) -> sk_rect_t
uint32_t SkPicture_uniqueID(reskia_picture_t *picture); // (SkPicture *picture) -> uint32_t
int SkPicture_serialize(reskia_picture_t *picture, const reskia_serial_procs_t *procs); // (SkPicture *picture, const SkSerialProcs *procs) -> sk_data_t
...
int SkPicture_makeShader(reskia_picture_t *picture, int tmx, int tmy, int mode, const reskia_matrix_t *localMatrix, const reskia_rect_t *tileRect); // (SkPicture *picture, SkTileMode tmx, SkTileMode tmy, SkFilterMode mode, const SkMatrix *localMatrix, const SkRect *tileRect) -> sk_shader_t
int SkPicture_makeShader_2(reskia_picture_t *picture, int tmx, int tmy, int mode); // (SkPicture *picture, SkTileMode tmx, SkTileMode tmy, SkFilterMode mode) -> sk_shader_t
...
int SkPicture_MakeFromStream(reskia_stream_t *stream, const reskia_deserial_procs_t *procs); // (SkStream *stream, const SkDeserialProcs *procs) -> sk_picture_t
int SkPicture_MakeFromData(const reskia_data_t *data, const reskia_deserial_procs_t *procs); // (const SkData *data, const SkDeserialProcs *procs) -> sk_picture_t
int SkPicture_MakeFromData_2(const void *data, size_t size, const reskia_deserial_procs_t *procs); // (const void *data, size_t size, const SkDeserialProcs *procs) -> sk_picture_t
