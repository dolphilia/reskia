...
sk_codec_t SkWbmpDecoder_Decode(int static_stream, void * result, void * decodeContext) {
    return static_sk_codec_make(SkWbmpDecoder::Decode(static_sk_stream_take_entity(static_stream), static_cast<SkCodec::Result *>(result), decodeContext));
}

sk_codec_t SkWbmpDecoder_Decode_2(int static_data, void * result, void * decodeContext) {
    return static_sk_codec_make(SkWbmpDecoder::Decode(static_sk_data_get_entity(static_data), static_cast<SkCodec::Result *>(result), decodeContext));
}
