...
void SkRuntimeEffect_delete(void *runtime_effect); // owned: caller が保持する参照を release する (SkRuntimeEffect *runtime_effect)
...
const void *SkRuntimeEffect_source(void *runtime_effect); // borrowed: 解放不要の借用ポインタ (SkRuntimeEffect *runtime_effect) -> const std::string *
...
const void *SkRuntimeEffect_findUniform(void *runtime_effect, int name); // borrowed: 解放不要の借用ポインタ (SkRuntimeEffect *runtime_effect, string_view_t name) -> const SkRuntimeEffect::Uniform *
const void *SkRuntimeEffect_findChild(void *runtime_effect, int name); // borrowed: 解放不要の借用ポインタ (SkRuntimeEffect *runtime_effect, string_view_t name) -> const SkRuntimeEffect::Child *
...
void SkRuntimeEffect_ref(void *runtime_effect); // retained: 参照カウントを増やす (SkRuntimeEffect *runtime_effect)
void SkRuntimeEffect_unref(void *runtime_effect); // owned: 参照カウントを減らす (SkRuntimeEffect *runtime_effect)
